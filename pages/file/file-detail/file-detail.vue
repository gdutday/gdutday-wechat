...
<view style="height: 100vh;" class="flex-column bg-white">
    <cu-custom :isBack="unShare"><template v-slot:content>文件详情</template></cu-custom>
    <view class="flex-column j-around flex-1">
        <view class="w-1 flex-column a-center" style="margin-top: 100px;padding: 0rpx 100rpx;">
            <image :src="iconPath" style="height: 65px;width: 65px;" />
            <text class="text-df mt-4 mb-2">{{ name}}</text>
            <text class="text-sm text-gray">未知大小</text>
        </view>
        <view class="flex-column a-center" style="padding: 0px 225rpx;">
            <button @tap="$refs.modal.showModal()" type="primary" :style="themeBorder + $themeBackground" class="w-1 mb-2">打开</button>
            <button type="primary" open-type="share" :style="themeBorder + $themeFont" class="w-1 bg-white">分享</button>
        </view>
    </view>
        ...
        <block v-else>
            <view class="cu-dialog " @tap.stop="" style="width:650rpx;">
                <block v-if="iconType != 'zip' && iconType != 'unknown'">
                    ...
                    <view @tap="openFile" class="hg flex-row text-df px-4">打开文件</view>
                </block>
                <view style="height: 1px;background:#EBEBEB;"></view>
...
import {
    iconType
} from '@/pages/file/file_commonFun.js';
    ...
    methods: {
        ...
        copyUrl() {
            ...
            uni.showModal({
                ...
                confirmText: '复制链接',
                success: e => e.confirm && uni.setClipboardData({
                    data: this.url
                })
            });
        }
