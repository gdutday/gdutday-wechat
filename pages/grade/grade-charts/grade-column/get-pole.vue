<template>
	<view>
		<view class="qiun-bg-white qiun-title-bar qiun-common-mt">
			<view class="qiun-title-dot-light">学科绩点排行</view>
		</view>
		<view class="qiun-charts">
			<canvas
				canvas-id="poleGradeColumn"
				id="poleGradeColumn"
				class="charts"
				@touchstart="touchColumn"
				@touchmove="moveColumn"
				@touchend="touchEndGetColumn"
			/>
		</view>
	</view>
</template>
	...
	computed: {
		sortPoleGrade() {
			return this.$commonFun
				.deepCopy(this.$store.getters.sortGradeByPole)
				.sort((a, b) => +b.examPole - a.examPole);
		}
	},
	created() {
	...
	methods: {
		init() {
			column = showColumn(
				this,
				'poleGradeColumn',
				grade2series(this.sortPoleGrade, item => +item.examPole),
				{
		...
		touchEndGetColumn(e) {
			...
			column.showToolTip(e, {
				format: (item, category) => {
					const num = (
						(+item.data.credit * item.data.examPole * 100) /
						this.$store.getters.scoresPole.get
					).toFixed(2);
					return `学分:${item.data.credit} 绩点:${
						item.data.examPole
					} 贡献率:${num}% ${item.data.examTerm} ${item.data.readMethod}`;
				}
			});
		}
	}
